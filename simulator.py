...
# Se ignora TRADE_PCT ya que la apuesta ahora es fija de $1.0
...
# ── Salida (Desactivadas) ──────────────────────────────────────────────────────
# Ponemos valores inalcanzables para que no gatillen salidas automáticas
TARGET_PRICE      = 1.10      
SL_PRICE_ABS      = -0.10     
...
class Trade:
    ...
    # No hay salidas por precio en esta estrategia
...
class Portfolio:
    ...
    def consider_entry(self, signal: dict, market_question: str,
        ...
        # Filtro de tiempo: Solo último minuto
        if secs_left > MAX_SECS_TO_ENTER or secs_left < MIN_SECS_TO_ENTER:
            return False

        # Seleccionar la opción Underdog (la más barata en el rango 0.05-0.15)
        direction = None
        entry_price = None

        # Priorizamos la que esté en el rango; si ambas lo están, la más barata
        ...
        # Elegir la de menor precio (menos probabilidades)
    ...
    def check_exits(self, *args, **kwargs) -> Optional[str]:
        # Sin Stop Loss ni Take Profit manual
        return None
    ...
    def close_trade(self, up_price: float, down_price: float) -> Optional[Trade]:
        ...
        # Ganamos si el precio del token que compramos termina en 1 (fue el ganador)
        won = (up_price >= 0.5) if trade.direction == "UP" else (down_price >= 0.5)
        
        pnl = trade.close_binary(won, 1.0 if won else 0.0)
        self.capital = round(self.capital + (trade.bet_size + pnl if won else 0), 4)
